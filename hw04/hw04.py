def shuffle(s):
    ...
    mid = len(s) // 2
    return [s[i // 2] if i % 2 == 0 else s[mid + i // 2] for i in range(len(s))]
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m), "must check if a mobile or a planet is balanced"
        left_end = end(left(m))
...
def prune_leaves(t, vals):
    ...
    if is_leaf(t) and label(t) in vals:
        ...
        new = [prune_leaves(b, vals) for b in branches(t)]
        new = [b for b in new if b is not None]

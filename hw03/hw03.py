...
def num_eights(num):
    ...
    if num == 0:
        return 0
    return (num % 10 == 8) + num_eights(num // 10)
...
def digit_distance(num):
    ...
    if num < 10:
        ...
        a = abs((num % 10) - ((num // 10) % 10)) + digit_distance(num //10)
    return a
...
def interleaved_sum(num, f_odd, f_even):
    ...
    def help(k, f, g):
        if k > num:
            return 0
        return f(k) + help(k + 1, g, f)
    return help(1, f_odd, f_even)
...
def count_dollars(sum_needed):
    ...
    def help(left, bill):
        ...
        if bill is None:
            return 0
        a = help(left - bill, bill)
        b = help(left, next_smaller_dollar(bill))
        return a + b
    return help(sum_needed, 100)
